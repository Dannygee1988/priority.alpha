/*
  ...
  1. New Tables
    - `social_posts`
      ...
      - `rns_document_id` (uuid, optional) - Reference to RNS if post is related
      - `campaign_id` (text) - For grouping related posts
      - `analytics` (jsonb) - Performance metrics
...
CREATE TABLE IF NOT EXISTS social_posts (
  ...
  CONSTRAINT content_length CHECK (
    CASE
      WHEN platform = 'twitter' THEN char_length(content) <= 280
