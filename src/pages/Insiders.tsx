...
import { Plus, Search, Filter, UserRound, Mail, Phone, Building2, MoreVertical, X, AlertCircle, Check, Tag as TagIcon, Trash2, ChevronDown, Wand2 } from 'lucide-react';
            ...
            {marketSoundings.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {marketSoundings.map((sounding) => (
                  <div
                    key={sounding.id}
                    className="p-4 rounded-lg border border-neutral-200 hover:bg-neutral-50"
                  >
                    <div className="space-y-2">
                      <div>
                        <span className="text-sm text-neutral-500">Title:</span>
                        <div className="font-semibold">{sounding.subject}</div>
                      </div>
                      <div>
                        <span className="text-sm text-neutral-500">Project:</span>
                        <div>{sounding.project_name}</div>
                      </div>
                      {sounding.expected_cleanse_date && (
                        <div>
                          <span className="text-sm text-neutral-500">Expected Cleanse:</span>
                          <div>{new Date(sounding.expected_cleanse_date).toLocaleDateString()}</div>
                        </div>
                      )}
                      <div>
                        <span className="text-sm text-neutral-500">Type:</span>
                        <div>{sounding.type}</div>
                      </div>
              ...
              <div className="text-center py-8">
                <p className="text-neutral-500">No market soundings</p>
              </div>
            )}
          </div>
        </div>
